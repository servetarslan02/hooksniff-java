...
import com.hooksniff.internalapi.EndpointAutoConfig;
...
public final class AutoConfig {
    ...
    private final HookSniff hooksniff;
    ...
    public AutoConfig(final String token, final EndpointIn endpointIn) throws InvalidTokenException {
        ...
        try {
            ...
            this.hooksniff = new HookSniff(content.getTokenPlaintext(), options);
        } catch (IllegalArgumentException e) {
            throw new InvalidTokenException(e);
        }
    }
    ...
    /** Registers this endpoint with HookSniff using the auto-config API. */
    public EndpointOut subscribe() throws IOException, ApiException {
        return new EndpointAutoConfig(hooksniff.getHttpClient())
