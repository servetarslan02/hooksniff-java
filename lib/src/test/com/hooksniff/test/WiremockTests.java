...
public class WiremockTests {
    ...
    public void testRetryForStatus500() throws Exception {
        ...
        String req_id = requests.get(0).getRequest().header("hooksniff-req-id").firstValue();

        for (int i = 0; i < requests.size(); i++) {
            // all retries have the same req id
            LoggedRequest req = requests.get(0).getRequest();
            assertEquals(req_id, req.header("hooksniff-req-id").firstValue());
            if (i == 0) {
                // first request does not have hooksniff-retry-count
                wireMockRule.verify(
                        1,
                        getRequestedFor(urlEqualTo("/api/v1/app"))
                                .withoutHeader("hooksniff-retry-count"));
            } else {
                wireMockRule.verify(
                        1,
                        getRequestedFor(urlEqualTo("/api/v1/app"))
                                .withHeader("hooksniff-retry-count", equalTo(Integer.toString(i))));
            }
        }
        ...
        wireMockRule.verify(
                4,
                getRequestedFor(urlEqualTo("/api/v1/app"))
                        .withHeader("hooksniff-req-id", equalTo(req_id)));
    }
    ...
    public void tokenAndUserAgentIsSent() throws Exception {
        ...
        wireMockRule.verify(
                1,
                deleteRequestedFor(urlEqualTo("/api/v1/app/app1"))
                        .withHeader("User-Agent", matching("hooksniff-libs/.*/java")));
    }
